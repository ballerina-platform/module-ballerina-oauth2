...
public class OAuth2Client {
    ...
    public static Object doHttpRequest(Environment env, BString url, BMap<BString, Object> clientConfig,
        ...
        int requestTimeout;
        try {
            connectionTimeout = getConnectionTimeoutInMillis(clientConfig);
            requestTimeout = getRequestTimeoutInMillis(clientConfig);
        } catch (BError e) {
            return e;
        }
    ...
    private static int getConnectionTimeoutInMillis(BMap<BString, ?> clientConfig) throws BError {
        double connectTimeoutInSeconds = clientConfig.containsKey(OAuth2Constants.CONNECTION_TIMEOUT) ?
                ((BDecimal) clientConfig.get(OAuth2Constants.CONNECTION_TIMEOUT)).floatValue() :
                ModuleUtils.getOauth2ConnectionTimeout();
        if (connectTimeoutInSeconds <= 0) {
            throw createError("OAuth2 connection timeout must be greater than zero");
        }
    ...
    private static int getRequestTimeoutInMillis(BMap<BString, ?> clientConfig) throws BError {
        double requestTimeoutInSeconds = clientConfig.containsKey(OAuth2Constants.REQUEST_TIMEOUT) ?
                ((BDecimal) clientConfig.get(OAuth2Constants.REQUEST_TIMEOUT)).floatValue() :
                ModuleUtils.getOauth2RequestTimeout();
        if (requestTimeoutInSeconds <= 0) {
            throw createError("OAuth2 request timeout must be greater than zero");
        }
